%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%processWIOT16.m produces wiot16_strc.mat                             %
...
nctry = length(countrycode);

% number of rows and cols in IO data 
[nrowitems, ncolitems] = size(TT.data);

cols = T.Properties.VariableNames;
finalCols = cols(nind*nctry+5:end-1);%+5 for the 4 cols of info, -1 total col
nfinals = size(finalCols,2)/nctry;
finalcat = finalCols(1:nfinals)';
...
wiot16 = wiot_raw;
wiot_wototals = wiot16(1:nctry*nind,1:(nctry*nind)+(nfinals*nctry),:);
wiot_cln = netInventCorrect(wiot_wototals,finalcat,nind,nctry);
...
Z = wiot_cln(1:nind*nctry,1:nind*nctry,:);

% F matrix with F_ij^r as a typical element (& 4 final categories per country)
F = wiot_cln(1:nind*nctry,(nind*nctry)+1:size(wiot_cln,2),:);
